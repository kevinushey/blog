...
of memory error called a segfault with its `memcheck`
tool.
...
catch a single large-mouth bass, when what you really
...
applicable to `gdb` as well -- they perform essentially
the same function, although `lldb` is somewhat behind
`gdb` in terms of advanced features.
...
If you then call the `ouch()` function, you should have
`lldb` immediately plop you down right where the
failure happened.
...
can call `bt` or `thread backtrace`:
...
come back to life (press ENTER again to get the prompt
to reappear):
...
Well, so be it! This is still fun. Let's step through a bit more.
...
something, but I'm not really sure when, but it
crashed, please fix").
...
then manually run your script. In other words, `R -d
lldb -f script.R` does not work!
...
that I very strongly suggest you read before executing
willy-nilly.
...
[this little script](https://github.com/kevinushey/etc/blob/master/R/Mac/r-lldb):
    ...
    : ${REXEC:=/Library/Frameworks/R.framework/Resources/bin/exec/R}
    
    R_HOME=${R_HOME} lldb --batch --file "${REXEC}" -o "process launch -- -f $1"
