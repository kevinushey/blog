...
In other words, some code we had introduced recently to change the background color in the toolbar was ultimately responsible for this issue. By scoping this code so that it would only ever run in macOS Mojave, the bug was gone and RStudio was again rendering as expected.

Now, with the benefit of hindsight, we can start looking into what side-effects `view.wantsLayer = YES;` might have that could cause this issue.
